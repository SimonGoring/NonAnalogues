...
* But now we are concerned about novelty during 20th century.  Both equilibrial and disequilibrial.
...
cor.est <- cor.test(rf$land, rf$self, use = 'pairwise.complete.obs', method='spearman')

ggplot(rf, aes(x = land, y = self)) + 
  geom_point(alpha = 0.5, aes(color = division)) + 
  scale_x_log10() + 
  scale_y_log10() + 
  geom_abline() + 
  coord_fixed(ratio=1) +
...
Turnover within the core is correlated to landscape dissimilarity (r<sub>s</sub> = `r p(cor.est$estimate)`, p < 0.001), but shows a great deal of variability around the 1:1 line (Figure X).  
...
compiled.to.indiv <- match(compiled.pollen$sitename, indiv.sites$site)
...
#  This also removes te "Humid Tropical" ecozone because these samples only span the
...
for(i in 1:1500){
  #  This loop basically pulls out the 95% CI samples.
  age.class <- rf$age %in% (i*10):((i*10)+500)
  aa <- quantile(rf$diss[age.class], probs=0.90, na.rm=TRUE)
  rf$nf[age.class & rf$diss > aa] <- 'red'
}

what <- rf$domain %in% c('HUMID TEMPERATE DOMAIN')

curve <- ggplot(aes(x = age, y = diss), data = rf[what,]) + 
...
points <- ggplot(data = data.frame(map), aes(long, lat)) + 
  ...
  geom_point(data = rf[rf$diss > quantile(rf$diss, 0.95) & what, ], 
             aes(x = long, y = lat, size = diss*10), alpha=0.2, color = 'red') +
  scale_size_identity() +
...
model0 <- gam(log(self,10) ~ 1, family = gaussian, 

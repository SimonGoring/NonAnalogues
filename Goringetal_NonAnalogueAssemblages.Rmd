...
full.frame <- data.frame(rbind(pollen.frame, climate.frame[clim.in.pol,]),
...
full.frame <- full.frame[!is.na(full.frame$min),]
...
cross.plot <- dcast(full.frame, 
                    list(.(site, age), .(variable)),
                    value.var = 'min', 
                    fun.aggregate = function(x)mean(x, na.rm=TRUE))

ggplot(cross.plot, aes(x = climate, y = pollen)) + 
  geom_point(alpha=0.1) + scale_x_sqrt() + scale_y_sqrt()
...
inclusion <- full.frame$age < 15000 & full.frame$age > -50 &
  full.frame$min > 0

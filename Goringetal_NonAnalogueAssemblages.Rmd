...
if('rep.frame.RData' %in% list.files('data/output')){
  load('data/output/rep.frame.RData')
}
if(!'rep.frame.RData' %in% list.files('data/output')){
...
ggplot(cross.plot, aes(x = land, y = self,color = direction)) + 
...
Turnover within the core is correlated to landscape dissimilarity ($r_{s}$ = `r p(cor.est$estimate)`, p < 0.001), but shows a great deal of variability around the 1:1 line (Figure X).  

...
rf <- data.frame(diss = rowMeans(rep.frame[!is.na(rep.frame[,4]),4:103], na.rm=TRUE), 
                 age = rep.frame$age[!is.na(rep.frame[,4])],
                 indiv.sites[match(rep.frame[!is.na(rep.frame[,4]),1], indiv.sites[,1]),2:3],
                 site = rep.frame$site[!is.na(rep.frame[,4])],
                 dataset = rep.frame$dataset[!is.na(rep.frame[,4])])

ninefive <- sort(rf$diss)[nrow(rf)*0.975]

curve <- ggplot(aes(x = age, y = diss), data = rf) + geom_point(alpha = 0.25) +
  geom_smooth(formula = y ~ s(x), method='gam', family = Gamma) +
